...
from .models import HelpAuditEvent
...
class HelpAuditLog:
    ...
    def record(self, actor: str, action: str, target: str, payload: dict | None = None) -> HelpAuditEvent:
        event = HelpAuditEvent(f"ha_{uuid.uuid4().hex[:12]}", actor, action, target, payload or {})
        with sqlite3.connect(self.db_path) as con:
            con.execute(
                """
                INSERT INTO help_audit_events
    ...
    def list_events(self, limit: int = 100) -> list[dict]:
        with sqlite3.connect(self.db_path) as con:
            rows = con.execute(
                ...
                FROM help_audit_events ORDER BY created_at DESC LIMIT ?
                """,
                (limit,),
            ).fetchall()
        return [

...
from .models import NotificationAuditEvent
...
class NotificationAuditLog:
    ...
    def record(self, actor: str, action: str, target: str, payload: dict | None = None) -> NotificationAuditEvent:
        event = NotificationAuditEvent(f"nae_{uuid.uuid4().hex[:12]}", actor, action, target, payload or {})
        with sqlite3.connect(self.db_path) as con:
            con.execute(
                """
    ...
    def list_events(self, limit: int = 100) -> list[dict]:
        with sqlite3.connect(self.db_path) as con:
            rows = con.execute(
                ...
                FROM notification_audit_events ORDER BY created_at DESC LIMIT ?
                """,
                (limit,),
            ).fetchall()
        return [

...
from .models import AnalyticsAuditEvent
...
class AnalyticsAuditLog:
    ...
    def record(self, actor: str, action: str, target: str, payload: dict | None = None) -> AnalyticsAuditEvent:
        event = AnalyticsAuditEvent(f"anae_{uuid.uuid4().hex[:12]}", actor, action, target, payload or {})
        with sqlite3.connect(self.db_path) as con:
            con.execute(
                """
    ...
    def list_events(self, limit: int = 100) -> list[dict]:
        with sqlite3.connect(self.db_path) as con:
            rows = con.execute(
                ...
                FROM analytics_audit_events ORDER BY created_at DESC LIMIT ?
                """,
                (limit,),
            ).fetchall()
        return [

...
from .models import ContentAuditEvent
...
class ContentAuditLog:
    def __init__(self, db_path: str | Path = "data/template-content-studio.db"):
        self.db_path = Path(db_path)
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        with sqlite3.connect(self.db_path) as con:
            con.executescript(SCHEMA)

    def record(self, actor: str, action: str, target: str, payload: dict | None = None) -> ContentAuditEvent:
        event = ContentAuditEvent(f"csae_{uuid.uuid4().hex[:12]}", actor, action, target, payload or {})
        with sqlite3.connect(self.db_path) as con:
            con.execute(
                """
    ...
    def list_events(self, limit: int = 100) -> list[dict]:
        with sqlite3.connect(self.db_path) as con:
            rows = con.execute(
                ...
                FROM content_audit_events ORDER BY created_at DESC LIMIT ?
                """,
                (limit,),
            ).fetchall()
        return [
